...
const updateUser = async (req, res) => {
  ...
  try {
    ...
    if (role && role == "admin") return res.status(400).send({msg : 'you can not change your role as admin'})
    
    const checkEmail = await User.findOne({email : email})
    if (checkEmail) return res.status(400).send({msg : 'try another email address'})
